...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template
        match="*:datafield[@tag = 'ZAT'] | datafield[@tag = 'CAT' or @tag = 'AF1']/*:subfield[@code = 'a']"/>
    ...
    <xsl:function name="flub:WriteOutXpath">
        ...
        <xsl:variable name="element-string">
            <xsl:for-each select="$current-element">
                <xsl:variable name="attribute_values" as="xs:string*">
                    <xsl:if test="@tag">
                        <xsl:sequence select="concat('@tag:', @tag)"/>
                    </xsl:if>
                    <xsl:if test="not(matches(@ind1, '^\s$')) and @ind1">
                        <xsl:sequence select="concat('@ind1:', @ind1)"/>
                    </xsl:if>
                    <xsl:if test="not(matches(@ind2, '^\s$')) and @ind2">
                        <xsl:sequence select="('@ind2:', @ind2)"/>
                    </xsl:if>
                    <xsl:if test="@code">
                        <xsl:sequence select="concat('@code:', @code)"/>
                    </xsl:if>
                </xsl:variable>

                <xsl:variable name="hasAttributes" as="xs:boolean"
                    select="
                        if (count($attribute_values[string(.)]) > 0) then
                            true()
                        else
                            false()"/>
                <xsl:sequence
                    select="
                        concat('/', name(), if ($hasAttributes) then
                            '['
                        else
                            '',
                        if (count($attribute_values) > 1) then
                            string-join($attribute_values, ' ')
                        else
                            $attribute_values
        ...
        <xsl:choose>
            <xsl:when test="$parent-element">
                <xsl:sequence
                    select="flub:WriteOutXpath($parent-element, concat($element-string, $sofar))"/>
            </xsl:when>
            <xsl:otherwise>

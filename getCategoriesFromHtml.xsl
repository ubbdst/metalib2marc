...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="sub-cat"
        match="*:td[@class = 'TableRow' and following-sibling::*:td/*:input/@name = 'sub_display']"
        use="."/>
    ...
    <xsl:template match="/">
            ...
            <root>
                <xsl:for-each
                    ...
                    <xsl:variable name="language">
                        <xsl:if test="not(matches($base-uri, '_[a-z]{2}\.html?'))">
                            <xsl:message terminate="yes">
                                <xsl:value-of select="$base-uri"/> missing language tag in AA
                                _AA.html? </xsl:message>
                        </xsl:if>
                        <xsl:value-of
        ...
        <root>
            <xsl:for-each select="$main-category-ids">
                <cat1 id="{.}">
                    <xsl:variable name="sub-display-inputs"
                        select="key('main-cat', ., $category-documents)/ancestor::*:form/descendant::*:input[@name = 'sub_display']"/>
                    <xsl:variable name="sub-categories-ids"
                        select="distinct-values($sub-display-inputs/parent::*/preceding-sibling::*:td[1])"/>

                    <xsl:for-each select="key('main-cat', ., $category-documents)">
                        <xsl:variable name="input-field"
                            select="following::*:input[@name = 'subcategory_display'][1]"/>
                        <xsl:attribute name="{$input-field/ancestor::*:doc/@lang}"
                            select="$input-field/@value"/>
                    </xsl:for-each>

                    <xsl:for-each select="$sub-categories-ids">
                        <xsl:variable name="sub-category"
                            select="key('sub-cat', ., $category-documents)"/>
                        <cat2 id="{.}">
                            <xsl:for-each select="$sub-category">
                                <xsl:attribute name="{ancestor::*:doc/@lang}">
                                    <xsl:value-of
                                        select="./following-sibling::*:td[1]/*:input[@name = 'sub_display']/@value"
                                    />
                                </xsl:attribute>
                            </xsl:for-each>
                        </cat2>

...
<xsl:template match="*" mode="copy">
    ...
    <!-- Removed because of unused fields in marc. Should some of these be remapped?
         Github issues.
    -->
    <xsl:template match="*:datafield[@tag=('531','532','574',
        '575','591','592','593','594','595','901','902','956') and *:subfield/@code='a']   |
        *:datafield[@tag='856']/*:subfield[@code=('$7','$D')]|
        *:datafield[@tag='245']/*:subfield[@code='$9']" priority="2.0" mode="copy #default"/>
...
<xsl:element name="{local-name()}">           <xsl:copy-of select="@*"/>
    ...
    <xsl:template match="*:datafield[@tag='520']|
        ...
        *:datafield[@tag='260' and (*:subfield/@code='a' or *:subfield/@code='b')]" priority="2.1">
   <xsl:element name="{local-name()}">
...
<xsl:template name="controlfield_008">
    ...
    -->
<xsl:template match="*:record[*:datafield[@tag='STA']/*:subfield[@code='a']='ACTIVE']" priority="1.0">
    <xsl:element name="{local-name()}">
        ...
        <xsl:variable name="conditionForOA" as="xs:boolean">
            <xsl:sequence select="if (*:datafield[@tag='594']/subfield[@code='a'
                and matches(.,'FREE','i')]
                )
                then true()
                else false()"/>
        ...
        <xsl:apply-templates select="parent::*:knowledge_unit/*:category/(*:main except *:main[preceding-sibling::*:main=.])" mode="datafield_920"/>
   ...
   <xsl:template match="*:datafield"></xsl:template>
    
   <xsl:template match="*:datafield[@tag='856' and @ind1='4' and (@ind2='1' or @ind2='9') and *:subfield/@code='u' ]" priority="2.5">
    ...
    <xsl:template mode="datafield_920" match="*:category/*:main">
        ...
        <xsl:for-each select="tokenize($languages,'\s')">
            ...
            <datafield tag="920" ind1=" " ind2=" ">
                <xsl:variable name="current-cat" select="key('category-by-id',$this_context,$categories)"/>
                <xsl:if test="not($current-cat/(@* except @id))">
                    <xsl:message terminate="yes">Finner inngen språk attributter for <xsl:value-of select="$this_context"/> Opprett 'en' eller 'nb' translations i <xsl:value-of select="base-uri($categories)"/></xsl:message>
                </xsl:if>                
                <subfield code="a"><xsl:value-of select="$current-cat/@*[name()=$lang]"/></subfield>            
                <xsl:variable name="sub-context" select="$this_context/following-sibling::*[1]"/>                    
                <xsl:for-each select="$this_context/following-sibling::*:sub[preceding-sibling::*[1]/text()=$this_context]">
                                      <xsl:variable name="sub-cat" select="key('category-by-id',.,$current-cat)/@*[name()=$lang]"/>
                <subfield code="b"><xsl:value-of select="$sub-cat"/></subfield>
                </xsl:for-each>               
                <xsl:call-template name="subfield_code_5"/>
                <subfield code="9"><xsl:value-of select="if (.='en') then 'eng' else 'nor'"/></subfield>
            </datafield>
        </xsl:for-each>    
    </xsl:template>    
        
    <!-- remove all datafields which are not covered by the general rule-->
    <xsl:template match="*:datafield" priority="0.7"></xsl:template>
        
    <!-- remove all records which are not ACTIVE-->
    <xsl:template match="*:record" priority ="0.7"></xsl:template>
...
<xsl:function name="flub:replaceFieldInPosition">
    ...
    <xsl:if test="not($controlfield/local-name()='controlfield')">
        <xsl:message terminate="yes">
            flub:replaceFieldInPosition:  not controlfield
        </xsl:message>
    </xsl:if>
    <!-- 008 first index position is 00, so starting position is -->
    <xsl:variable name="marc-position" select="$position" />
    <xsl:variable name="regexp" select="concat('^([\s\S]{',$marc-position,'})[\s\S]{',$length,'}')"/>
   ...
   <xsl:variable name="controlfield_out" as="node()">
    ...
    <xsl:if test="$position &lt; 0 or $position &gt; string-length($controlfield)">
        <xsl:message terminate="yes">flub:replaceFieldInPosition $position <xsl:value-of select="$position"/> out of bounds. </xsl:message>
    </xsl:if>
    
    <xsl:if test="string-length($controlfield) != string-length($controlfield_out)">
        <xsl:message terminate="yes">
           flub:replaceFieldInPosition(): controlfield out of bounds.
        </xsl:message>
    </xsl:if>
  ...
  <xsl:template name="subfield_code_5">
        <subfield code="5"><xsl:value-of select="$institution"/></subfield>
    </xsl:template>
